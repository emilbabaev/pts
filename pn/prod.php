<?
	...
	if (isset($_GET['price']))
	{
		...
		if (basename($_FILES['name']['name']) != '')
		{
			...
			move_uploaded_file($_FILES['name']['tmp_name'], $_SERVER['DOCUMENT_ROOT'].'/load/catalog.csv');
		}
		$SQL = "UPDATE `files` SET `name`='".$name."' WHERE `id`=1";
		$result = mysql_query($SQL);
		$handle = fopen($_SERVER['DOCUMENT_ROOT']."/load/catalog.csv", "r");
		while (($data = fgetcsv($handle, 1000, ";")) !== FALSE) {
			...
			if ($data[0] == '') {
				...
				mysql_query("UPDATE `prod` SET `name`='".mysql_real_escape_string(iconv("Windows-1251", "UTF-8", $data[1]))."', `price`='".(double)str_replace(",", ".", $data[8])."', `weight`='".(double)str_replace(",", ".", $data[5])."', `count_pallet`='".(double)str_replace(",", ".", $data[3])."' ".$color.$size.$brand_strength.$producer." WHERE `id`=".$data[0]."");
			}
	...
	if (isset($_GET['del']))
	{
        $SQL = "DELETE FROM link_cats WHERE id_prod = '".$_GET['del']."';";
        $result = mysql_query($SQL);
	...
	if (isset($_GET['name']) && $_GET['name'] != "") {
		$search_str .= " and (`prod`.`name` LIKE '%".$_GET['name']."%') ";
		
	}
	if (isset($_GET['id_category']) && $_GET['id_category'] != "") {
		$search_str .= " and (`prod`.`id_category`=".$_GET['id_category']." or `category`.`parentId`=".$_GET['id_category'].") ";
	}
	$SQL = "SELECT * FROM `".$table."`, `category` WHERE `category`.`id`=`prod`.`id_category` ".$search_str."";
	$result = mysql_query($SQL);
...
?>
		  ...
		  <div class="col-md-6">
			...
			<div id="myTabContent" class="tab-content">
			  ...
			  <form id="tab" action="/pn/<?echo $page_url;?>.php?page=<?=$_GET['page'];?><?if (isset($_GET["name"])) {echo "&id_category=".$_GET["id_category"]."&name=".$_GET["name"];}?>" method="post"  enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <?
						$categories = mysql_query("SELECT id, name FROM `category`");
						$selected_categories = mysql_query("SELECT id_category FROM link_cats WHERE id_prod = ".$_GET["edit"]);

						while ($cat = mysql_fetch_assoc($categories)) $_categories[$cat["id"]] = $cat;
						while ($sl = mysql_fetch_assoc($selected_categories)) $_slcategories[$sl["id_category"]] = $sl;

						foreach ($_slcategories as $key => $_scat)
						{
							$_categories[$key]["selected"] = true;
						}
						ksort($_categories);
                    ?>
                    <label>Дополнительные категории</label>
                    <select name="scategory_id[]" class="form-control" multiple size="7">
                        <option value="0"></option>
                        <? foreach ($_categories as $_cat): ?>
                            <option value="<?=$_cat["id"];?>" <?=($_cat["selected"]) ? "selected" : "" ?> ><?=$_cat["name"];?></option>
						<? endforeach; ?>
                    </select>
                </div>
                ...
                <div class="form-group">
                  <label style="vertical-align:top;line-height:2.2;">Сортировка</label>
		  ...
		  <div class="col-md-6">
			...
			<div id="myTabContent" class="tab-content">
			  ...
			  <form id="tab" action="/pn/<?echo $page_url;?>.php" method="post" enctype="multipart/form-data">
				...
				<select name="id_category" class="form-control">
                  ...
                  <div class="form-group">
                      ...
                      <select name="scategory_id[]" class="form-control" multiple size="7">
                          <option value="0"></option>
                          <?
                          $res = mysql_query("SELECT id, name FROM `category` ORDER by name ASC");
                ...
                <div class="form-group">
                  <label style="vertical-align:top;line-height:2.2;">Сортировка</label>
                  <input type="number" name="sort" class="form-control" value="999">
                </div>
				<div class="form-group">
				<label>Описание</label>
				<textarea name="text" class="form-control"><?echo $text;?></textarea>
			  ...
			  <a href="/pn/<?echo $page_url;?>.php" class="btn btn-danger">Отмена</a>
				...
				<?
					...
					if (isset($_POST['add_data']))
					{
						...
						$SQL = "INSERT INTO `".$table."` (`id`, `id_category`, `id_manufacturer`, `id_size`, `id_brand_strength`, `id_color`, `name`, `text`, `price`, `img`, `weight`, `count_pallet`, `status`, `url`, `enable`, `new`, `special`, `new_price`, `sort`) VALUES (NULL, '".$_POST['id_category']."', '".$_POST['id_manufacturer']."', '".$_POST['id_size']."', '".$_POST['id_brand_strength']."', '".$_POST['id_color']."', '".mysql_real_escape_string($_POST['name'])."', '".mysql_real_escape_string($_POST['text'])."', '".(double)($_POST['price'])."', '".$img."', '".(double)($_POST['weight'])."', '".(double)($_POST['count_pallet'])."', '".$_POST['status']."', '".$url."', '1', '".(int)($_POST['new'])."', '".(int)($_POST['special'])."', '".(double)($_POST['new_price'])."', '".$_POST['sort']."');";
						$result = mysql_query($SQL);

                        $lastitem = mysql_insert_id();
                        // Добавление в subcats
                        $data_cats = $_POST["scategory_id"];
                        foreach ($data_cats as $cid)
                        {
                            if ($cid != 0)
                            {
                                $SQL = "INSERT INTO link_cats (id_prod, id_category) VALUES ('".$lastitem."', '".$cid."')";
                                $push = mysql_query($SQL);
                            }
                        }
					...
					{
					    // Добавление в subcats
                        $SQL = "DELETE FROM link_cats WHERE id_prod = '".$_POST['edit_data']."';";
                        $result = mysql_query($SQL);
                        ...
                        foreach ($data_cats as $cid)
                        {
                            if ($cid != 0)
                            {
                                $SQL = "INSERT INTO link_cats (id_prod, id_category) VALUES ('".$_POST['edit_data']."', '".$cid."')";
                                $result = mysql_query($SQL);
                            }
                        }
			...
			$SQL = "SELECT `prod`.`id`, `prod`.`name`, `prod`.`sort`, `prod`.`img` FROM `".$table."`, `category` WHERE `category`.`id`=`prod`.`id_category` ".$search_str." ORDER BY `prod`.`id` DESC LIMIT ".$count_data.", 20";
			$result = mysql_query($SQL);
		...
		function loadCsv(){
			$.ajax({
				...
				cache: false,			
				success: function(response){
					location.href = "/load/catalog.csv";
				}
			});
		}
	</script>	  
