...
include $_SERVER['DOCUMENT_ROOT']."/pn/bd.php";
	...
	$SQL = "SELECT `prod`.`name`, `prod`.`id`, `prod`.`price`, `prod`.`count_pallet`, `prod`.`weight`, `producer`.`name` as `producer_name`, `size`.`name` as `size_name`, `brand_strength`.`name` as `brand_strength_name`, `color`.`name` as `color_name`
	FROM `prod` 
	...
	$handle = fopen($_SERVER['DOCUMENT_ROOT']."/load/catalog.csv", "w+");
